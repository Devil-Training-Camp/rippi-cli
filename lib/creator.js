import { fetchRepoList, fetchRepoTime } from './request.js';
import {
  loading, loadModule, writeFileTree, injectImports, getConfigurations, getInstallPkgCommand, getRunStartCommand, shouldUpdateTemplate,
} from './utils.js';
...
class Creator {
  ...
  async chooseTool() {
    const { tool } = await inquirer.prompt({
      ...
      type: 'list',
      message: '请选择项目的工具',
      choices: ['npm', 'pnpm', 'yarn'].map((choice) => ({ name: choice, value: choice })),
    });
    // 检测是否已安装该工具
    try {
      await execa(tool, ['--version']);
    } catch (error) {
      const { shouldInstallTool } = await inquirer.prompt({
        name: 'shouldInstallTool',
        type: 'confirm',
        message: `检测到您未安装${chalk.red(tool)}，是否全局安装${chalk.red(tool)}？`,
      });
      if (shouldInstallTool) {
        await execa('npm', ['install', '-g', tool]);
      } else {
        return;
      }
    }
  ...
  async startProject(tool) {
    ...
    if (shouldStart) {
      await execa(tool, getRunStartCommand(tool));
    } else {
  ...
  // 仓库最后一次commit的时间
  fetchRepoUpdateTime = async () => {
    const branchInfo = await loading(fetchRepoTime, 'Checking if the template needs to be updated');
    const date = branchInfo.commit.commit.author.date;
    return date;
  }

  download = async (branch) => {
    // 拼接下载路径 这里放自己的模板仓库url
    // 这个 branch 需要编码一下吧？比如：`feat/xxx` 这种带了 `/` 的分支名，就会产生新的目录
    const requestUrl = `rippi-cli-template/react/#${branch}`;
    // 把资源下载到指定的本地磁盘的文件夹
    const localCacheFolder = userhome('.rippiorg_templates');
    // 指定文件夹的模板的路径
    const localTemplateCacheUrl = (this.templateDir = path.join(localCacheFolder, 'react', branch));
    // 判断是否已经下载过该模板
    // 模板可能有更新吧，咋整？有没有可能更精确地判断要不要拉最新模板代码呢？
    const hasDownloaded = fs.existsSync(localTemplateCacheUrl);
    let flag = false;
    // 如果已经下载过模板，比较本地模板的创建时间与模板的最后一次commit的时间，若是早于commit时间，说明更新模板
    if (hasDownloaded) {
      const date = await this.fetchRepoUpdateTime();
      flag = await shouldUpdateTemplate(localTemplateCacheUrl, date);
    }
    if (!hasDownloaded || flag) {
  ...
  create = async () => {
    ...
    // 内部插件名集合
    const innerPlugins = Object.keys(projectOptions.plugins);
    // 外部插件，先将于内部插件重名的去掉
    const outerPlugin = (configurations.plugins ?? []).filter((plugin) => !innerPlugins.includes(innerPlugins));
    outerPlugin.forEach((outerPlugin) => {
    ...
    // 获取安装指定包的命令
    const installPkgCmd = getInstallPkgCommand(tool);
    await execa(tool, [installPkgCmd, ...pluginDeps, '-D'], orderConfig);
    // 6 解析和收集插件
    ...
    // 10 安装所有依赖
    await execa(tool, ['install'], orderConfig);
    await this.startProject(tool);
  }
};
