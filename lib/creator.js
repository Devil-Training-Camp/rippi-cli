import { fetchRepoTime } from './request.js';
import {
...
class Creator {
  ...
  async startProject(tool) {
    ...
    if (shouldStart) {
      await execa(tool, getRunStartCommand(tool), { cwd: this.dir, stdio: 'inherit' });
    } else {
  ...
  create = async () => {
    ...
    // 外部插件，先将于内部插件重名的去掉
    const outerPlugin = (configurations.plugins ?? []).filter(() => !innerPlugins.includes(innerPlugins));
