...
export const writeFileTree = (projectDir, files) => {
  Object.keys(files).forEach((file) => {
    ...
    // ejs 有什么特殊的呢？为什么要 slice？
    // 还有，你这里 `-4` 是一个魔数，更好的方法应该是：
    // file.slice(0, -'.ejs'.length);
...
// 根据不同的工具获取不同的安装指定包的命令
export const getInstallPkgCommand = (tool) => {
  if (tool === 'npm') {
    return 'install';
  }
  return 'add';
...
// 获取启动项目的命令
export const getRunStartCommand = (tool) => {
  if (tool === 'npm') {
    return ['run', 'dev'];
  }
  return ['dev'];
...
export const shouldUpdateTemplate = (url, updateTime) => new Promise((resolve) => {
  fs.stat(url, (err, stat) => {
    resolve(Date.parse(stat.birthtime) < Date.parse(updateTime));
  })
});
